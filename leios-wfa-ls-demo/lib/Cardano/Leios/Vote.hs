...
createNonPersistentVote nonce committee privKey@(PrivateKeyLeios (nId, sk)) eId ebHash = case findNonPersistentVoterByPublicKey pk vtrs of
  ...
  where
    vtrs = voters . nonPersistentVoters $ committee
    pk = PublicKeyLeios (nId, deriveVerKeyDSIGN sk) :: PublicKeyLeios 'Vote
